...
client-----left switch1 ====Bottle Link1 =====rightswitch1----... server nodes 1
    |                                           
    -------left switch2   ====Bottle Link2 ======rightswitch2----...server nodes 2
...
class DumbbellTopo(Topo):
    ...
    def build(self, server_number, client_link_opt: dict, bot_nec_link_opt: [dict], server_link_opts: [dict]):
        ...
        client_s1 = self.addSwitch('s0')
        left_router_bo1 = self.addSwitch('s1')
        right_router_bo1 = self.addSwitch('s2')
        left_router_bo2 = self.addSwitch('s3')
        right_router_bo2 = self.addSwitch('s4')
        server_nodes = []
        ...
        self.addLink(tcp_left_node, left_router_bo2, **{"bw": 1000, "delay": '2ms'})
        # connect tcpright node with right switch
        self.addLink(right_router_bo2, tcp_right_node, **{"bw": 1000, "delay": '2ms'})
        self.switch_idx = 5
        info('*** Creating links\n')
        # connect client and left router
        self.addTCLink(client_node, client_s1, **client_link_opt)
        self.addTCLink(client_s1, left_router_bo1, **client_link_opt)
        self.addTCLink(client_s1, left_router_bo2, **client_link_opt)
        # connect left and right router
        self.addTCLink(left_router_bo1, right_router_bo1, **bot_nec_link_opt[0])
        self.addTCLink(left_router_bo2, right_router_bo2, **bot_nec_link_opt[1])
        # connect right router and server

        for i in range(server_num-2):
            print(i)
            self.addTCLink(right_router_bo1, server_nodes[i], **server_link_opts[i])
        for i in range(server_num-2,server_number):
            print(i)
            self.addTCLink(right_router_bo2, server_nodes[i], **server_link_opts[i])
        info(" network connected \n")
        self.client_node = client_node
        self.server_nodes = server_nodes
        self.left_router = [left_router_bo1,left_router_bo2]
        self.right_router = [right_router_bo1,right_router_bo2]
...
if __name__ == '__main__':
    ...
    topo = DumbbellTopo(server_number=server_num,
                        ...
                        bot_nec_link_opt=[btlink_opts1,btlink_opts2],
